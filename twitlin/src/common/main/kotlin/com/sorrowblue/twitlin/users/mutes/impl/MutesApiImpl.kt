package com.sorrowblue.twitlin.users.mutes.impl
...
import com.sorrowblue.twitlin.utils.API_MUTES
...
internal class MutesApiImpl(private val client: TwitterClient) : MutesApi {
    ...
    private suspend fun createImpl(userId: UserId?, screenName: String?): Response<User> {
        return client.submitForm("$API_MUTES/users/create.json", Response.serializer(User.serializer()), Parameters.build {
    ...
    private suspend fun destroyImpl(userId: UserId?, screenName: String?): Response<User> {
        return client.submitForm("$API_MUTES/users/destroy.json", Response.serializer(User.serializer()), Parameters.build {
            userId?.let { append("user_id", it.id) }
            screenName?.let { append("screen_name", it) }
        })
    }
