...
import com.sorrowblue.twitlin.v2.lists.Expansion
import com.sorrowblue.twitlin.v2.lists.ListsApi
...
import com.sorrowblue.twitlin.v2.tweets.Expansion as TweetsExpansion
...
internal class ListsApiImpl(private val client: TwitterClient) : ListsApi {
    ...
    override suspend fun list(
        ...
        return client.get("${Endpoints.LISTS}/${id.id}", Response.serializer(Lists.serializer())) {
    ...
    override suspend fun create(name: String, description: String, isPrivate: Boolean): Response<OptionalData<Lists>> {
        return client.post(Endpoints.LISTS, Response.serializer(OptionalData.serializer(Lists.serializer()))) {
            contentType(ContentType.Application.Json)
