...
import com.sorrowblue.twitlin.v2.Endpoints.USERS
...
import com.sorrowblue.twitlin.v2.lists.impl.DataResponse
import com.sorrowblue.twitlin.v2.lists.impl.ListFollowOrPinRequest
...
import com.sorrowblue.twitlin.v2.users.request.TargetUserIdRequest
...
import com.sorrowblue.twitlin.v2.objects.List as Lists
...
internal class UsersApiImpl(private val client: TwitterClient) : UsersApi {
    ...
    override suspend fun following(id: UserId, targetUserId: UserId): Response<Following> {
        return client.post("$USERS/${id.id}/following", Response.serializer(FollowingResponse.serializer())) {
            contentType(ContentType.Application.Json)
            body = TargetUserIdRequest(targetUserId)
        }.convertData { it.data }
    }
    ...
    override suspend fun followedLists(
        id: UserId,
        expansions: List<com.sorrowblue.twitlin.v2.lists.Expansion>?,
        listField: List<ListField>?,
        userFields: List<UserField>?,
        maxResults: Int,
        paginationToken: String?
    ...
    override suspend fun followList(id: UserId, listId: ListId): Response<Boolean> {
        return client.post(
            ...
            contentType(ContentType.Application.Json)
            body = ListFollowOrPinRequest(listId)
        }.convertData { it.data.following }
    }
