package com.sorrowblue.twitlin.users.blocks.impl
...
import com.sorrowblue.twitlin.utils.API_BLOCKS
...
internal class BlocksApiImpl(private val client: TwitterClient) : BlocksApi {
    ...
    private suspend fun createImpl(
        ...
        return client.submitForm("$API_BLOCKS/create.json", Response.serializer(User.serializer()), Parameters.build {
    ...
    private suspend fun destroyImpl(
        ...
        return client.submitForm("$API_BLOCKS/destroy.json", Response.serializer(User.serializer()), Parameters.build {
            userId?.let { append("user_id", it.id) }
            screenName?.let { append("screen_name", it) }
            append("include_entities", includeEntities.toString())
            append("skip_status", skipStatus.toString())
