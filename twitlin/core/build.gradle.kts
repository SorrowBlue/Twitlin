...
buildkonfig {
    packageName = "com.sorrowblue.twitlin.core"
    defaultConfigs {
        com.android.build.gradle.internal.cxx.configure.gradleLocalProperties(rootDir).toList()
            .filter { it.first.toString().startsWith("TWITTER_API_") }
            .forEach {
                buildConfigField(
